...
class DiplomacyMixin:
    ...
    def get_diplomacy(self) -> dict:
        ...
        if RUST_BRIDGE_AVAILABLE:
            try:
                ...
                logger.warning(
                    f"Rust parser failed for diplomacy: {e}, falling back to regex"
                )
    ...
    def _get_diplomacy_regex(self) -> dict:
        ...
        for match in re.finditer(r"\brelation\s*=\s*\{", rel_block):
            ...
            # Resolve empire name
            relation_info["empire_name"] = country_names.get(
                country_id, f"Empire {country_id}"
            )
    ...
    def _get_federation_details_regex(self) -> dict:
        ...
        members_match = re.search(
            r"\bmembers\s*=\s*\{([^}]*)\}", federation_block, re.DOTALL
        )
        if members_match:
            member_ids = [int(x) for x in re.findall(r"\d+", members_match.group(1))]
            result["members"] = member_ids

        progression_block = self._extract_braced_block(
            federation_block, "federation_progression"
        )
            ...
            if laws_block:
                ...
                if open_brace != -1 and close_brace != -1 and close_brace > open_brace:
                    ...
                    for key, value in re.findall(
                        r"\b([A-Za-z0-9_]+)=([A-Za-z0-9_]+)\b", inner
                    ):
                        result["laws"][key] = value
    ...
    def get_galactic_community(
        ...
        return self._get_galactic_community_regex(
            limit_members, limit_resolutions, limit_council
        )
    ...
    def _get_galactic_community_regex(
        ...
        def parse_int_list(block_key: str) -> list[int]:
            ...
            if open_brace == -1 or close_brace == -1 or close_brace <= open_brace:
                return []
            values = [
                int(x) for x in re.findall(r"\d+", block[open_brace + 1 : close_brace])
            ]
    ...
    def _get_subjects_regex(self, limit: int = 25) -> dict:
        ...
        start = inner_match.start() + (
            1 if agreements_section[inner_match.start()] == "\n" else 0
        )
        brace_count = 0
        end = None
        ...
        def parse_terms(term_data_block: str) -> dict:
            ...
            for str_key in [
                ...
                m = re.search(
                    rf"\b{re.escape(str_key)}=([A-Za-z0-9_]+)\b", term_data_block
                )
                if m:
            ...
            discrete_terms_block = self._extract_braced_block(
                term_data_block, "discrete_terms"
            )
            if discrete_terms_block:
                pairs = re.findall(
                    r"\bkey=([A-Za-z0-9_]+)\s*value=([A-Za-z0-9_]+)",
                    discrete_terms_block,
                )
                if pairs:
                    terms["discrete_terms"] = {k: v for k, v in pairs}

            resource_terms_block = self._extract_braced_block(
                term_data_block, "resource_terms"
            )
            if resource_terms_block:
                pairs = re.findall(
                    r"\bkey=([A-Za-z0-9_]+)\s*value=([-\d.]+)", resource_terms_block
                )
                if pairs:
        ...
        for match in re.finditer(r"\n\t\t(\d+)\s*=\s*\{", agreements_block):
            ...
            entry = {
                ...
                "date_changed": (
                    date_changed_match.group(1) if date_changed_match else None
                ),
                "preset": term_data.get("agreement_preset"),
                "specialization": specialization,
                "specialization_level": spec_level,
    ...
    def _get_fallen_empires_regex(self) -> dict:
        ...
        country_entries = [
            (int(m.group(1)), country_start + m.start())
        ...
        fe_positions = [
            (m.start(), "dormant")
            for m in re.finditer(r'type="fallen_empire"', self.gamestate)
        ]
        fe_positions += [
            (m.start(), "awakened")
            for m in re.finditer(r'type="awakened_fallen_empire"', self.gamestate)
        ]
        ...
        for fe_pos, status in fe_positions:
            ...
            block = (
                self.gamestate[country_pos:next_pos]
                if next_pos
                else self.gamestate[country_pos : country_pos + 200000]
            )
            ...
            opinion_m = re.search(
                r"relations_manager=.*?country=0[^}]*opinion=([-\d]+)",
                block[:80000],
                re.DOTALL,
            )
            if opinion_m:
    ...
    def get_claims(self) -> dict:
        ...
        go_section = self.gamestate[
            go_start : go_start + 10000000
        ]  # 10MB should cover most saves
