...
def build_event_state_from_briefing(briefing: dict[str, Any]) -> dict[str, Any]:
    ...
    economy = (
        briefing.get("economy") if isinstance(briefing.get("economy"), dict) else {}
    )
    territory = (
        briefing.get("territory") if isinstance(briefing.get("territory"), dict) else {}
    )
    technology = (
        briefing.get("technology")
        if isinstance(briefing.get("technology"), dict)
        else {}
    )
    diplomacy = (
        briefing.get("diplomacy") if isinstance(briefing.get("diplomacy"), dict) else {}
    )
    ...
    territory_state = (
        {"colonies": {"total_count": colonies.get("total_count")}}
        if colonies
        else {"colonies": {}}
    )
    ...
    technology_state = {
        k: technology.get(k) for k in ("tech_count",) if k in technology
    }
    diplomacy_state = {}
    ...
    history = (
        briefing.get("history") if isinstance(briefing.get("history"), dict) else {}
    )

    event_state: dict[str, Any] = {
        "meta": {
            k: meta.get(k)
            for k in ("date", "empire_name", "campaign_id", "player_id")
            if k in meta
        },
...
def record_snapshot_from_briefing(
    *,
    ...
    history = (
        briefing.get("history") if isinstance(briefing.get("history"), dict) else None
    )
    ...
    inserted, snapshot_id = db.insert_snapshot_if_new(
        ...
        wars_count=(
            wars.get("count") if isinstance(wars, dict) else metrics.get("wars_count")
        ),
...
def record_snapshot_from_companion(
    *,
    ...
    resolved_player_id = (
        player_id if player_id is not None else metrics.get("player_id")
    )
    ...
    full_json = json.dumps(
        briefing_for_storage, ensure_ascii=False, separators=(",", ":")
    )
    inserted, snapshot_id = db.insert_snapshot_if_new(
        session_id=session_id,
        game_date=metrics.get("game_date"),
