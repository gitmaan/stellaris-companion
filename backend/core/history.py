...
def extract_snapshot_metrics(briefing: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "military_power": safe_int(military.get("military_power")),
        "colony_count": safe_int(colonies.get("total_count")),
        # wars_count is not populated in the compact summary payload yet.
        "wars_count": None,
        "energy_net": safe_float(net.get("energy")),
        "alloys_net": safe_float(net.get("alloys")),
    }
...
def build_history_enrichment(
    *,
    ...
    # Ascension perks from signals (added in CHR-003)
    ascension_perks = signals.get("ascension_perks")
    ...
    # L-Gate status from signals (added in CHR-005)
    lgate = signals.get("lgate")

    # Menace/Become the Crisis status from signals (added in CHR-007)
    menace = signals.get("menace")

    # Great Khan / Marauder status from signals (added in CHR-012)
    great_khan = signals.get("great_khan")

    # Galactic Community status from signals (added in CHR-014)
    galactic_community = signals.get("galactic_community")

    # Traditions status from signals (added in CHR-016)
    traditions = signals.get("traditions")

    # Precursor discovery status from signals (added in CHR-018)
    precursors = signals.get("precursors")

    # Subject/vassal relationships from signals
    subjects = signals.get("subjects")

    # Strategic geography from signals
    geography = signals.get("geography")
    ...
    if ascension_perks:
        history["ascension_perks"] = ascension_perks
    if lgate:
        history["lgate"] = lgate
    if menace:
        history["menace"] = menace
    if great_khan:
        history["great_khan"] = great_khan
    if galactic_community:
        history["galactic_community"] = galactic_community
    if traditions:
        history["traditions"] = traditions
    if precursors:
        history["precursors"] = precursors
    if subjects:
...
def record_snapshot_from_briefing(
    *,
    ...
    full_json = (
        briefing_json if isinstance(briefing_json, str) and briefing_json else json_dumps(briefing)
    )
...
def record_snapshot_from_companion(
    *,
    ...
    full_json = json_dumps(briefing_for_storage)
    inserted, snapshot_id = db.insert_snapshot_if_new(
        session_id=session_id,
        game_date=metrics.get("game_date"),
        save_hash=save_hash,
