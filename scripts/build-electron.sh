...
# Check if dist-python has the executable
if [ ! -f "dist-python/stellaris-backend" ] && [ ! -f "dist-python/stellaris-backend.exe" ]; then
    echo "Error: Python backend executable not found in dist-python/"
    echo "Run ./scripts/build-python.sh first"
    exit 1
fi
