    ...
    for year in sorted(by_year.keys()):
        ...
        if len(year_events) > 10:
            ...
            for e in year_events:
                ...
                if t in ('war_started', 'war_ended', 'crisis_started', 'fallen_empire_awakened',
...
def summarize_state(briefing: dict, label: str = "CURRENT") -> str:
    ...
    if fe.get('war_in_heaven'):
        lines.append(f"WAR IN HEAVEN: Active")
...
def build_chronicler_prompt(data: dict) -> str:
    ...
    if identity.get('is_machine'):
        voice_note = "Write with cold, logical precision. No emotion, only analysis of historical patterns."
        ...
        voice_note = "Write valuing peace and diplomacy. Frame conflicts as tragedies, peace as triumph."
    elif 'spiritualist' in ethics or 'fanatic_spiritualist' in ethics:
        voice_note = "Write with religious reverence. Frame history as divine providence and sacred destiny."
...
def main():
    ...
    for i, s in enumerate(sessions):
        print(f"  {i+1}. {s['empire_name']} ({s['first_date']} → {s['last_game_date']}) - {s['event_count']} events")
